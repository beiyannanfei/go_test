...
func TestPay(t *testing.T) {
	...
	tests := []struct {
		...
		//{"android_qq", args{request: DoMidasPayReq{
		...
		{"android_wechat", args{request: DoMidasPayReq{
		...
		//{"ios_qq", args{request: DoMidasPayReq{
		//	OpenId:    "C64B7ECD63B5BAF7E1AAC4984777E89F",
		...
		//	AppRemark: "midas pay remark",
		//}}, nil, errors.NewCodeString(errors.AuthTokenInvalid, "token校验失败(18)")},
		//{"ios_wechat", args{request: DoMidasPayReq{
		//	OpenId:    "oNB9uwjqf-ufdJjUI5RkJCHSlcPU",
		...
		//	AppRemark: "midas pay remark",
		//}}, nil, errors.NewCodeString(errors.AuthTokenInvalid, "token校验失败(18)")},
		//{"ios_qq_test", args{request: DoMidasPayReq{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, got1 := Pay(tt.args.request)
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("Pay() got = %v, want %v", got, tt.want)
			}
			if !reflect.DeepEqual(got1, tt.want1) {
				t.Errorf("Pay() got1 = %v, want %v", got1, tt.want1)
			}
		})
	}
}
