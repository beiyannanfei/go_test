...
// Binding from JSON
// noinspection ALL
type DoMidasGetBalanceReq struct {
	//
	// 腾讯登录态：
...
type DoMidasGetBalanceReqBodyParams struct {
	//
	// swagger:allOf
	// in: body
	DoMidasGetBalanceReq DoMidasGetBalanceReq
}

//
// 应答: 扣除游戏币接口
// swagger:response DoMidasGetBalanceRsp
// noinspection ALL
type DoMidasGetBalanceRsp struct {
	// in: body
	Body struct {
		//
		...
		// 历史累计收到赠送金额
		//
		tss_list map[string]interface{} `json:"tss_list"`
	}
}

func GetBalance(request DoMidasGetBalanceReq) (*DoMidasGetBalanceRsp, error) {
