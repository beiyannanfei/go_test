...
type DoMidasPresentReq struct {
	...
	// 赠送游戏币的个数(注：不能为0)
	//
	PresentTimes int `json:"presenttimes" structs:"presenttimes"`
	//
	...
	// （可选）用户的外网IP
	//
	UserIp int `json:"userip" structs:"userip"`
}

//
// in: body
// swagger:parameters profile_query_vip
type DoMidasPresentReqBodyParams struct {
	//
	// swagger:allOf
	// in: body
	DoMidasPresentReq DoMidasPresentReq
}

//
// 应答: 直接赠送接口
// swagger:response DoMidasPresentRsp
// noinspection ALL
type DoMidasPresentRsp struct {
	// in: body
	Body struct {
		//
...
func Present(request DoMidasPresentReq) (*DoMidasPresentRsp, error) {

	params := structs.Map(&request)

	method := "GET"
	//noinspection SpellCheckingInspection
	urlPath := "/mpay/present_m"
