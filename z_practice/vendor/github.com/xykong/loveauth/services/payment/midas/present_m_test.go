...
func TestPresent(t *testing.T) {
	...
	tests := []struct {
		name  string
		args  args
		want  *DoMidasPresentRsp
		want1 error
	}{
		//{"android_qq", args{request: DoMidasPresentReq{
		...
		{"android_wechat", args{request: DoMidasPresentReq{
		...
		//{"ios_qq", args{request: DoMidasPresentReq{
		//	OpenId:       "C64B7ECD63B5BAF7E1AAC4984777E89F",
		...
		//	BillNo:       "1234567890",
		//}}, nil, errors.NewCodeString(errors.AuthTokenInvalid, "token校验失败(18)")},
		//{"ios_wechat", args{request: DoMidasPresentReq{
		//	OpenId:       "oNB9uwjqf-ufdJjUI5RkJCHSlcPU",
		...
		//	BillNo:       "1234567890",
		//}}, nil, errors.NewCodeString(errors.AuthTokenInvalid, "token校验失败(18)")},
		//{"ios_qq_test", args{request: DoMidasPresentReq{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, got1 := Present(tt.args.request)
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("Present() got = %v, want %v", got, tt.want)
			}
			if !reflect.DeepEqual(got1, tt.want1) {
				t.Errorf("Present() got1 = %v, want %v", got1, tt.want1)
			}
		})
	}
}
