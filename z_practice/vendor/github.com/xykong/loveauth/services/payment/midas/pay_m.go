...
type DoMidasPayReq struct {
	...
	//
	// （可选）备注。会写到账户流水。长度限制256字节
	//
	AppRemark string `json:"appremark" structs:"appremark"`
}

//
// in: body
// swagger:parameters profile_query_vip
type DoMidasPayReqBodyParams struct {
	//
	// swagger:allOf
	// in: body
	DoMidasPayReq DoMidasPayReq
}

//
// 应答: 扣除游戏币接口
// swagger:response DoMidasPayRsp
// noinspection ALL
type DoMidasPayRsp struct {
	// in: body
	Body struct {
		//
		// 返回码 0：成功；1018：登录态校验失败；1001：参数错误；1034错误码 (余额不足)；其他：失败
		//
		Ret int `json:"ret"`
		//
		...
		Msg string `json:"msg"`
		//
		// 预扣流水号（调接口时有传就返回的业务传的billno），该流水号有效期48小时
		//
		...
		// 本次扣的赠送币的金额
		//
		UsedGenAmount int `json:"used_gen_amt"`
	}
}

func Pay(request DoMidasPayReq) (*DoMidasPayRsp, error) {
