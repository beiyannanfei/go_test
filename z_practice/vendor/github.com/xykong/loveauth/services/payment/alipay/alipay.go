...
var privateKey []byte
var aliPayPublicKey []byte

func Init() error {

	var err error

	privateKey, err = initPrivateKey(settings.GetString("lovepay", "alipay.privateKey"))
	aliPayPublicKey, err = initPublicKey(settings.GetString("lovepay", "alipay.publicKey"))

	return err
}

func initPrivateKey(path string) ([]byte, error) {
	...
	return encoding.ParsePrivateKey(string(body)), nil
}

func initPublicKey(path string) ([]byte, error) {
	...
	return encoding.ParsePublicKey(string(body)), nil
}

func URLValues(param AliPayParam) (value url.Values, err error) {

	appId := settings.GetString("lovepay", "alipay.appId")
	//privateKey := encoding.ParsePrivateKey(settings.GetString("lovepay", "alipay.privateKey"))
	var p = url.Values{}
	p.Add("app_id", appId)
	p.Add("method", param.APIName())
	p.Add("format", K_FORMAT)
	p.Add("charset", K_CHARSET)
	p.Add("sign_type", K_SIGN_TYPE_RSA2)
	...
	sign, err := signWithPKCS1v15(p, privateKey, crypto.SHA256)
	if err != nil {
		return nil, err
	}
	p.Add("sign", sign)
...
func doRequest(request string, param AliPayParam, results interface{}) (err error) {
	...
	resp, err := http.Post(request, K_CONTENT_TYPE_FORM, buf)
	if resp != nil {
	...
	//aliPayPublicKey := encoding.ParsePublicKey(settings.GetString("lovepay", "alipay.aliPayPublicKey"))
	if len(aliPayPublicKey) > 0 {
		...
		if sign != "" {
			if ok, err := verifyData([]byte(content), K_SIGN_TYPE_RSA2, sign, aliPayPublicKey); ok == false {
				return err
			}
		}
	}
...
func VerifySign(data url.Values) (ok bool, err error) {
	return verifySign(data, aliPayPublicKey)
}
